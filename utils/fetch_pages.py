...
def wait_until_ready(session, page):
    ...
    for attempt in range(MAX_WAIT_TIME // SLEEP_INTERVAL):
        try:
            response = session.get(BASE_URL, params=params, timeout=10)
            elapsed = response.elapsed.total_seconds()
            match = expected_text in response.text

            print(f"[Seite {page}] Versuch {attempt+1}: {elapsed:.3f}s – Match: {match}")

            if response.status_code == 200 and match:
                return True, response.text, elapsed
        except requests.RequestException as e:
            print(f"[Seite {page}] Fehler: {e}")
...
def fetch_pages():
    with requests.Session() as session:
        ...
        for page in tqdm(range(START_PAGE, END_PAGE - 1, -1), desc="Seiten abfragen"):
            ...
            if success:
                tqdm.write(f"✅ Seite {page} vollständig – Antwortzeit: {response_time:.2f}s, Gesamt: {total_page_time:.2f}s")
            else:
